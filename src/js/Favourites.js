...
export default class Favourites {
  ...
  addToFavourites(event) {
    ...
    const isFavouriteMark = event.target.closest('.media_messages_message').querySelector('.media_message_favourite');
    if (isFavouriteMark) {
      return;
    }
  ...
  showFavouriteMark(messageId) {
    ...
    const favouriteElement = DrawUI.getFavouriteMark();
    messageElement[0].querySelector('.media_message_header').prepend(favouriteElement);
  ...
  removeFavouriteMark(messageId) {
    const messageElement = [...this.messageClass.messages.entries()]
      .filter(({ 1: id }) => id === messageId).map(([key]) => key);
    messageElement[0].querySelector('.media_message_favourite').remove();
  }
}
